	 		   ...
	 		   if(checkedBoxs.length < 1){
	 			   ...
	 			   $.ligerDialog.confirm("确认要删除吗?","删除职位",function(r){
	 				   if(r){
	 					   // alert("删除："+ids.get());
	 					   // 发送请求
	 					   window.location = "${ctx }/job/deleteJob?ids=" + ids.get();
	 				   }
	 			   });
	 		   }
	 	   })
	...
	<table width="100%" height="90%" border="0" cellpadding="5" cellspacing="0" class="main_tabbor">
	  ...
	  <tr valign="top">
	    <td height="30">
		  <table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
		    <tr>
			  <td class="fftd">
			  	<form name="jobform" method="post" id="jobform" action="${ctx}/job/findJobByParam">
				    <table width="100%" border="0" cellpadding="0" cellspacing="0">
					  <tr>
					    <td class="font3">
			...
			<c:forEach items="${requestScope.jobList}" var="job" varStatus="stat">
				<tr id="data_${stat.index}" align="center" class="main_trbg" onMouseOver="move(this);" onMouseOut="out(this);">
					<td><input type="checkbox" id="box_${stat.index}" value="${job.id}"></td>
					 <td>${job.name }</td>
					  <td>${job.remark }</td>
					 <td align="center" width="40px;"><a href="${ctx}/job/showUpdateJob?id=${job.id}">
	  ...
	  <tr valign="top"><td align="center" class="font3">
	  	 <fkjava:pager
	  	        ...
	  	        style="digg"
	  	        submitUrl="${ctx}/job/findJobByParam?pageIndex={0}"/>
	  </td></tr>
	</table>
